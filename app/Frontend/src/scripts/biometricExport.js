...
/**
 * Crea un botón de exportación en la UI
 */
function createExportButton(containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;

    const buttonHTML = `
        <button 
            id="btn-export-biometrics" 
            class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-semibold flex items-center gap-2"
            title="Exportar datos biométricos a Excel"
        >
            <span>📊</span>
    ...
    const button = document.getElementById('btn-export-biometrics');
    if (button) {
