...
// =====================================================
// CONFIGURACIÓN LCD (modo paralelo sin I2C)
// =====================================================
const int RS = 12;
const int EN = 11;
const int D4 = 5;
...
// VARIABLES LCD
// =====================================================
String lcdLines[4];  // últimas 4 líneas mostradas

void actualizarLCD(const String &nuevoTexto)
{
    // Desplaza las líneas hacia arriba
    for (int i = 0; i < 3; i++) {
        lcdLines[i] = lcdLines[i + 1];
    }
    lcdLines[3] = nuevoTexto;

    // Refresca pantalla
    lcd.clear();
    for (int i = 0; i < 4; i++) {
        lcd.setCursor(0, i);
        if (lcdLines[i].length() > 16)
            lcd.print(lcdLines[i].substring(0, 16));
        else
...
// =====================================================
// FUNCIÓN: Recibir comandos tipo C1XXXF
// =====================================================
void recibirComandos()
{
    if (Serial.available() >= 6)
    {
        ...
        if ((trama.length() == 6 || trama.length() == 7) && trama.endsWith("F"))
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (!isDigit(codigoStr[i])) { valido = false; break; }
            }
            ...
            if (valido && (cabina == "C1" || cabina == "C2"))
            {
                ...
                actualizarLCD(msg); // ← Mostrar en pantalla LCD
...
void procesarComando(String cabina, int codigo)
{
    if (codigo >= 0 && codigo <= 17)
    {
        ...
        {
            digitalWrite(13, LOW);
            actualizarLCD("Apagar " + cabina);
        }
        else
        {
            digitalWrite(13, HIGH);
            actualizarLCD("Encender " + cabina);
        }
    }
    else if (codigo >= 39 && codigo <= 88)
    {
        actualizarLCD("Sonido " + String(codigo));
    }
    else if (codigo == 35)
    {
        digitalWrite(13, HIGH);
    ...
    {
        digitalWrite(13, LOW);
        actualizarLCD("Cod:" + String(codigo));
    }
}